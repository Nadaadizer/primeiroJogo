...
class Level:
    def __init__(self, window, name, game_mode):
        ...
        self.last_heal_time = -30  # para poder usar logo no início
        pygame.time.set_timer(EVENT_ENEMY, SPAWN_TIME)

    def save_progress(self):
        with open("save.txt", "w") as file:
            file.write(
                f"{self.player.level},{self.player.xp},{self.player.next_xp},{self.partidas_jogadas},{int(self.best_time)}\n")
    ...
    def load_progress(self):
        try:
            with open("save.txt", "r") as file:
                ...
                print(
                    f"Save carregado: LVL {self.player.level}, XP {self.player.xp}/{self.player.next_xp}, Partidas: {self.partidas_jogadas}, Melhor tempo: {self.best_time}s")
        except:
            pass
    ...
    def run(self):
        ...
        while running:
            ...
            # Regen de stamina
            self.player.stamina = min(self.player.stamina + (2 * dt), 20)
            ...
            # HUD
            cooldown_restante = max(0, 30 - (time.time() - self.last_heal_time))
            heal_status = f"Pronto" if cooldown_restante == 0 else f"{int(cooldown_restante)}s"
            self.level_text(26,
                            f'HP:{self.player.hp} XP:{self.player.xp}/{self.player.next_xp} LVL:{self.player.level} Time:{int(self.current_run_time)}s STA:{int(self.player.stamina)}/20 Poção:{heal_status}',
                            COLOR_WHITE, (10, WIN_H - 20))
